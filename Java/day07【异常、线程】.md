...
class MyThread extends Thread {
    ...
    public void run() {
        // 线程执行
        for (int i = 0; i < 500; i++) {
            System.out.println("子线程" + this.i + "打印:" + i);
        }
    }
}
```

注意必须调用子线程类的start方法，如果直接调用run方法，则还是一个线程（主线程）
...
5. Runnable接口中定义的是子线程实现逻辑（run方法）
...
public class Main {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        // 将Callable对象 包装一个未来任务对象
        // 其实就是Runnable对象，包装成Thread，处理子线程的返回值
        FutureTask<Integer> task = new FutureTask<>(mt);
        Thread t = new Thread(task, "my-task");
        t.start();
...
public class Main {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        // 开两个线程模拟两个人取钱
        // 通过有参构造器传共享资源
        Thread xiaoming = new MyThread(account, "小红");
        xiaoming.start();

        Thread xiaohong = new MyThread(account, "小明");
        xiaohong.start();
...
class Account {
    ...
    public void drawMoney(int money) {
        if (this.money >= money) {
            System.out.println(Thread.currentThread().getName() + "取了100000块钱");
            this.money -= money;
            System.out.println(Thread.currentThread().getName() + "取钱后余" + this.money);
        } else {
...
问题：this.money -= money，可能第一个线程没走完，第二个线程就进来了
...
为了保证每个线程都能正常执行原子操作,Java引入了线程同步机制（加锁），对进入临界资源的代码进行加锁。
...
缺点在于两个线程必须访问的是同一个共享对象。

## 5.4 同步方法

![img_2.png](img_2.png)- **同步方法**:使用synchronized修饰的方法,就叫做同步方法,保证A线程执行该方法的时候,其他线程只能在方法外等着。
