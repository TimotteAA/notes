...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 关掉线程池
        pools.shutdown(); // 任务完成后再关闭
        pools.shutdownNow(); // 立刻关闭
    }
}
...
线程池也可以用Callable对象做任务，并可从返回的Future类对象中获得结果。
```java
public class Main {
...
class ThreadDead {
    ...
    public static void test() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                synchronized (resource1) {
                    System.out.println("线程一占用资源一，请求资源二");
                    // 睡一会，让第二个线程去拿资源
                    try {
                        Thread.sleep(1000);
                    } catch (InterruptedException e) {
                    ...
                    synchronized (resource2) {
                        System.out.println("线程一占用资源二，请求资源一");
                    }
                }
            }
        }).start();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                synchronized (resource2) {
                    System.out.println("线程二占用资源二，请求资源一");
                    // 睡一会，让第一个线程去拿资源
                    try {
                        Thread.sleep(1000);
                    } catch (InterruptedException e) {
                    ...
                    synchronized (resource1) {
                        System.out.println("线程二占用资源一，请求资源二");
                    }
                }
            }
        }).start();
...
public void countDown()	// 计数器进行减1，尝试唤醒等待的线程
```
