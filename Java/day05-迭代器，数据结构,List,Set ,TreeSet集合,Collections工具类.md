...
#### 左旋与右旋
1. 左子树高，将节点右旋（左子节点成为父节点）
2. 右子树高，将节点左旋（右子节点成为父节点）
3. 如果左旋不行，则对右子树右旋；反之如果右旋不行，则左子树左旋
...
public class Main {
    ...
    public static void main(String[] args) {
        TreeSet<Person> pset = new TreeSet<>((o1, o2) -> {
            if (o2.age > o1.age) return 1;
            return -1;
        });

        pset.add(new Person(1111));
        pset.add(new Person(123121));
        pset.add(new Person(-222));
